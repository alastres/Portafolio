                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                    ...
                    <aside className="lg:col-span-2 hidden lg:block">
                        <div className="sticky top-32 space-y-8">
                            <div>
                                <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">{t('project_page.on_this_page')}</h3>
                                <nav className="flex flex-col space-y-3">
                                    {[
                                        { label: t('project_page.summary'), id: 'resumen' },
                                        { label: t('project_page.problem'), id: 'el-problema' },
                                        { label: t('project_page.solution'), id: 'la-solución' },
                                        { label: t('project_page.stack'), id: 'stack-tecnológico' },
                                        { label: t('project_page.challenges'), id: 'retos-técnicos' }
                                    ].map((item) => (
                                        <a
                                            key={item.id}
                                            href={`#${item.id}`}
                                ...
                                {project.timeline && (
                                    <div>
                                        <h4 className="flex items-center text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2 gap-2">
                                            <Calendar className="h-3 w-3" /> {t('project_page.timeline')}
                                        </h4>
                                        <p className="text-sm font-medium text-foreground">{t(project.timeline)}</p>
                                    </div>
                                )}
                                {project.role && (
                                    <div>
                                        <h4 className="flex items-center text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2 gap-2">
                                            <User className="h-3 w-3" /> {t('project_page.role')}
                                        </h4>
                                        <p className="text-sm font-medium text-foreground">{t(project.role)}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-10 space-y-20">
                                    ...
                                    {project.link && (
                                        <Button asChild size="lg" className="h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold px-8">
                                            <Link href={project.link} target="_blank">
                                                {t('project_page.view_demo')} <ArrowRight className="ml-2 h-4 w-4" />
                                            </Link>
                                        </Button>
                                    )}
                                    {project.github && (
                                        <Button asChild variant="outline" size="lg" className="h-12 border-border text-foreground hover:bg-muted font-medium px-8">
                                            <Link href={project.github} target="_blank">
                                                <Github className="mr-2 h-4 w-4" /> {t('project_page.repository')}
                                            </Link>
                                        </Button>
                                    )}
                                </div>
                                ...
                                {(project.gallery && project.gallery.length > 0) ? (
                                    ...
                                    <div className="aspect-video w-full bg-secondary/50 flex items-center justify-center border border-border/50">
                                        <div className="text-center p-12">
                                            ...
                                            <p className="text-muted-foreground">{t('project_page.image_unavailable')}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ...
                        {project.problem && (
                            <section id="el-problema" className="grid md:grid-cols-[200px_1fr] gap-8 border-t border-border pt-16">
                                <div>
                                    ...
                                    <h2 className="text-2xl font-bold text-foreground">{t('project_page.problem')}</h2>
                                </div>
                                <div className="space-y-6">
                                    <p className="text-lg text-muted-foreground leading-relaxed">
                                        {t(project.problem.description)}
                                    </p>
                        ...
                        {project.solution && (
                            <section id="la-solución" className="grid md:grid-cols-[200px_1fr] gap-8 border-t border-border pt-16">
                                <div>
                                    ...
                                    <h2 className="text-2xl font-bold text-foreground">{t('project_page.solution')}</h2>
                                </div>
                                <div className="space-y-8">
                                    <p className="text-lg text-muted-foreground leading-relaxed">
                                        {t(project.solution.description)}
                                    </p>
                        ...
                        {project.techStack && (
                            <section id="stack-tecnológico" className="space-y-8 border-t border-border pt-16">
                                <h2 className="text-2xl font-bold text-foreground">{t('project_page.stack')}</h2>
                        ...
                        {project.challenges && (
                            <section id="retos-técnicos" className="space-y-12 border-t border-border pt-16">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h2 className="text-2xl font-bold text-foreground">{t('project_page.challenges')}</h2>
                                </div>
                                    ...
                                    {project.challenges.map((challenge, i) => (
                                        <div key={i} className="grid lg:grid-cols-2 gap-8 items-start">
                                            <div className="space-y-4">
                                                <h3 className="text-xl font-bold text-foreground">{t(challenge.title)}</h3>
                        ...
                        <div className="flex justify-between border-t border-border pt-16 gap-6">
                            {previous ? (
                                ...
                                >
                                    ...
                                    <h4 className="text-lg font-bold text-foreground flex items-center gap-2 group-hover:underline">
                                        <ArrowLeft className="h-4 w-4" /> {t(previous.title)}
                                    </h4>
                                </Link>
                            ) : <div></div>}
                            ...
                            {next ? (
                                ...
                                >
                                    ...
                                    <h4 className="text-lg font-bold text-foreground flex items-center gap-2 group-hover:underline">
                                        {t(next.title)} <ArrowRight className="h-4 w-4" />
                                    </h4>
                                </Link>
                            ) : <div></div>}
                        </div>
